...
def get_ping_results(ping_command, module, transport, kwargs):
    try:
        if transport == 'cli':
            ping = module.execute(ping_command, **kwargs)[0]
        elif transport == 'nxapi':
            response = module.execute(ping_command, **kwargs)
            ping = response['ins_api']['outputs']['output']['body']
...
def main():
    ...
    if module.params['transport'] == 'nxapi':
        kwargs['command_type'] = 'cli_show_ascii'
    ...
    ping_results, summary, rtt = get_ping_results(
                        ping_command, module, module.params['transport'], kwargs)
